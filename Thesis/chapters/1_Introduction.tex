...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/sample_rgb_501.jpg}
        \caption{Raw RGB angiography image}
        \label{fig:sample_rgb}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/sample_mask_501.jpg}
        \caption{Corresponding binary ground truth mask}
        \label{fig:sample_mask}
    \end{subfigure}
    \caption{Representative example from the CAM angiography dataset. (a) The raw RGB image of a chorioallantoic membrane (CAM) captured under a stereozoom microscope, showing a dense network of blood vessels. (b) The corresponding binary ground truth mask generated via adaptive Gaussian thresholding, where black pixels represent segmented vessel structures and white pixels represent background tissue.}
    \label{fig:sample_angiography}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{CAM Assay Biological Validation:} The complete pipeline is applied to a controlled \textit{in vivo} Chorioallantoic Membrane (CAM) assay experiment, demonstrating the system's ability to quantify dose-dependent angiogenic effects of carbon dots across multiple concentrations and timepoints, establishing direct relevance to tumor angiogenesis research.
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[block] (input) {RGB Angiography\\Image};
        \node[block, right=of input] (preprocess) {Preprocessing\\(Resize, Normalize)};
        \node[block, right=of preprocess] (segment) {Deep Learning\\Segmentation\\(UNet++ / DeepLabV3+)};
        \node[block, right=of segment] (skeleton) {Morphological\\Skeletonization\\(Zhang-Suen)};
        \node[block, right=of skeleton] (output) {Clinical Report\\(Networks, Branches,\\Endpoints)};
        
        \draw[arrow] (input) -- (preprocess);
        \draw[arrow] (preprocess) -- (segment);
        \draw[arrow] (segment) -- (skeleton);
        \draw[arrow] (skeleton) -- (output);
    \end{tikzpicture}
    \caption{High-level overview of the proposed end-to-end automated vessel segmentation and morphological analysis pipeline. Raw RGB angiography images are preprocessed, segmented using deep learning architectures, skeletonized using morphological thinning, and finally analyzed to extract quantitative vascular biomarkers.}
