...
Each RGB image, denoted as $I \in \mathbb{R}^{H \times W \times 3}$, is accompanied by an expert-annotated binary ground truth mask $G \in \{0,1\}^{H \times W}$. These ground truth masks were prepared and validated in a prior laboratory study using a systematic multi-stage pipeline. First, the green color channel was extracted from the raw RGB images to maximize the contrast of the hemoglobin-rich vasculature against the surrounding chorioallantoic membrane tissue. An initial coarse segmentation was performed using localized adaptive Gaussian thresholding to binarize the vessel boundaries. Due to non-uniform background illumination and salt-and-pepper noise inherent in microscopic imaging, this automated step introduced background artifacts and fractured thin capillary segments. To establish high-fidelity targets, these raw masks were imported into ImageJ (Fiji) for manual and semi-automated refinement by lab experts. Utilizing ImageJ's region-selection, thresholding adjustments, and pixel-editing brushes, the experts corrected the binary masks to repair fractured capillary junctions, resolve ambiguous vessel crossings, and delete false-positive background noise. AngioTool was further utilized to cross-verify the topological continuity and branch connections of the refined vessel tracts. This hybrid manual-digital workflow ensured that even extremely thin capillaries (often less than 3 pixels in width) were preserved with pixel-perfect topological continuity. Finally, to ensure computational efficiency and standardized batch processing during network training, all input images and their corresponding masks were resized to a uniform spatial resolution of $256 \times 256$ pixels using bilinear interpolation for the images and nearest-neighbor interpolation for the masks to preserve discrete binary boundaries.
